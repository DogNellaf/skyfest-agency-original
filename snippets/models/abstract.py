...
class BaseQuerySet(QuerySet):
    def published(self):
        return self.filter(status__exact=StatusChoices.PUBLIC.value)

    def hidden(self):
        return self.filter(status__exact=StatusChoices.HIDDEN.value)

    def draft(self):
        return self.filter(status__exact=StatusChoices.DRAFT.value)
...
class StatusOrderingModel(models.Model):
    ...
    status = models.SmallIntegerField(
        'Статус', default=StatusChoices.PUBLIC.value, choices=StatusChoices.choices
    )
...
class BaseDictionaryModel(BaseModel):
    title = models.CharField('Заголовок', max_length=400, unique=True)
